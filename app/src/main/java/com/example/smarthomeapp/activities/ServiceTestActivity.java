        ...
        startServiceBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (client == null) {
                    ...
                    if (devices.length == 0) {
                        Log.d(TAG1, "band isn't pared with you phone");
                        Toast.makeText(getBaseContext(), "Band isn't paired with your phone.", Toast.LENGTH_LONG).show();
                    }
                    client = BandClientManager.getInstance().create(getBaseContext(), devices[0]);
                }

                Log.d(TAG1, "Band is connecting...");
                if(client.getSensorManager().getCurrentHeartRateConsent() != UserConsent.GRANTED) {
                    client.getSensorManager().requestHeartRateConsent(ServiceTestActivity.this, heartRateConsentListener);
                }
                if(client.getSensorManager().getCurrentHeartRateConsent() == UserConsent.GRANTED) {
